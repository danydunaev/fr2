...
`notes-app` — отдельный мини-проект заметок с напоминаниями, WebSocket-синхронизацией и push-уведомлениями.

### Где находится

- `notes-app/` — UI, Service Worker, офлайн-логика
- `notes-app/server/server.js` — backend для Socket.IO и push API

### Порты

- `https://localhost:3002` — UI
- `https://localhost:3003` — backend

### Возможности

- Создание заметок и заметок с напоминанием
- Синхронизация между вкладками и браузерами
- Push-уведомления
- Отложить напоминание на 5 минут
- Обновление времени в списке заметок после snooze
- Fallback-уведомление в активной вкладке
...
### Основные backend эндпоинты notes-app
